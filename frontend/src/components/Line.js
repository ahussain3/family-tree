...
export default class LineTo extends Component {
    ...
    getPointForAnchor(id, anchorName, within) {
        const element = this.findElement(id)
        const box = element.getBoundingClientRect()

        const anchor = this.parseAnchor(anchorName)
        ...
        const x = box.left + box.width * anchor.x + offsetX;
        const y = box.top + box.height * anchor.y + offsetY;
    ...
    render() {
        ...
        const fromPoints = this.getPointForAnchor(this.props.from, this.props.fromAnchor)
        const toPoints = this.getPointForAnchor(this.props.to, this.props.toAnchor)
        return points ? (
            <Line {...fromPoints} {...toPoints} {...this.props} />
        ) : null;
    }
}
...
// ============================================================================================== //
// Awais's code below this line //

export class DrawLines extends React.Component {
    componentWillMount() {
        this.fromAnchor = this.parseAnchor(this.props.fromAnchor);
        this.toAnchor = this.parseAnchor(this.props.toAnchor);
        this.delay = this.parseDelay(this.props.delay);
    ...
    renderChildre(midPoint) {
        const childPoints = this.props.children.map(child => this.getPointForAnchor(child, "top"))

        return childPoints.map((childPoint, key) => {
            return <SteppedLine x0={midPoint.x} y0={midPoint.y} x1={childPoint.x} y1={childPoint.y} key={key} {...this.props} delay={10}/>
        })
    }

    render() {
        const fromPoints = this.getPointForAnchor(this.props.parent_a, "right")
        const toPoints = this.getPointForAnchor(this.props.parent_b, "left")
        const midPoint = {x: (fromPoints.x + toPoints.x) / 2, y: (fromPoints.y + toPoints.y) / 2}

        return fromPoints ? (<>
            <Line x0={fromPoints.x} y0={fromPoints.y} x1={toPoints.x} y1={toPoints.y} {...this.props} delay={10}/>
            {this.renderChildre(midPoint)}
        </>) : null;
    }
}

export class DrawLinesTo extends LineTo {
    render() {
        return <DrawLines {...this.props}></DrawLines>
    }
}